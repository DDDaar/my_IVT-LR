...
#   bash qwen_grpo_multi_gpu.sh 8 args/qwen_grpo.yaml 29531 deepspeed
...
LAUNCHER="${4:-auto}"   # auto | deepspeed | accelerate | torchrun
...
DEEPSPEED_CONFIG_PATH="$(python - "${CONFIG_PATH}" <<'PY'
import sys
path = sys.argv[1]
try:
    import yaml
    ...
    print("" if ds is None else str(ds))
except Exception:
    print("")
PY
)"

# Fallback to repo-local ZeRO-2 config if YAML doesn't provide one.
if [ -z "${DEEPSPEED_CONFIG_PATH}" ]; then
  DEEPSPEED_CONFIG_PATH="${SCRIPT_DIR}/ds_config_grpo.json"
fi
if [ -n "${DEEPSPEED_CONFIG_PATH}" ] && [ "${DEEPSPEED_CONFIG_PATH#/}" = "${DEEPSPEED_CONFIG_PATH}" ]; then
  DEEPSPEED_CONFIG_PATH="${SCRIPT_DIR}/${DEEPSPEED_CONFIG_PATH}"
fi
...
if [ "${LAUNCHER}" = "auto" ]; then
  if [ -n "${DEEPSPEED_CONFIG_PATH}" ] && [ -f "${DEEPSPEED_CONFIG_PATH}" ]; then
    LAUNCHER="deepspeed"
  elif [ "${NPU_MODE}" = "1" ]; then
    LAUNCHER="torchrun"
  else
    LAUNCHER="accelerate"
  fi
...
echo "[INFO] deepspeed_config=${DEEPSPEED_CONFIG_PATH}"
...
if [ "${LAUNCHER}" = "deepspeed" ]; then
  ...
  if [ ! -f "${DEEPSPEED_CONFIG_PATH}" ]; then
    echo "[ERROR] DeepSpeed config not found: ${DEEPSPEED_CONFIG_PATH}"
    exit 1
  fi
  if [ "${NPU_MODE}" = "1" ]; then
  ...
  echo "[ERROR] Unknown launcher: ${LAUNCHER}. Use auto/deepspeed/accelerate/torchrun."
  exit 1
fi
